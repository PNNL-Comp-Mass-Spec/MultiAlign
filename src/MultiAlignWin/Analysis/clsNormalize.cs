...
namespace MultiAlignWin
{
	...
	public class clsNormalize
	{
		...
		{
			MultiAlign.Features.clsClusterData clusterData = analysis.UMCData.mobjClusterData ; 
		...
		int GetGroups(ref int []groupIndices, clsMultiAlignAnalysis analysis)
		{
			...
			MultiAlign.clsDatasetInfo [] arrDatasetInfo = (MultiAlign.clsDatasetInfo []) 
				analysis.Files.ToArray(typeof(MultiAlign.clsDatasetInfo));
			if (arrDatasetInfo == null || MultiAlign.clsDatasetInfo.mintNumFactorsSpecified == 0)
			{
			...
			else
			{
				...
				{
					MultiAlign.clsDatasetInfo datasetInfo = arrDatasetInfo[fileNum] ; 
