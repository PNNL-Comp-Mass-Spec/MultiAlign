...
namespace MultiAlignWin
{
	...
	public class frmScatterPlot : System.Windows.Forms.Form
	{
		...
		{		
			...
			long [] mapping = this.mobjAnalysis.FactorTree.DatasetMapping;
		...
		{	
			...
			{	
				...
				ArrayList oldLeaves = mctlScatterPlot.HorizontalLabel.GetLeaves();				
				
				mctlScatterPlot.VerticalLabel.Root	 = labelRoot;
				mctlScatterPlot.HorizontalLabel.Root = labelRoot;
				mctlScatterPlot.HorizontalLabel.MinLeafHeight = 100;
				mctlScatterPlot.VerticalLabel.MinLeafHeight = 100;									
				mctlScatterPlot.HorizontalLabel.Init();  			
				mctlScatterPlot.VerticalLabel.Init();  				
												
				int numDatasets = mobjAnalysis.UMCData.NumDatasets;		
				int numClusters = mobjAnalysis.UMCData.mobjClusterData.NumClusters ; 		
				float [,] tempData	= new float[numClusters, numDatasets];				
				long [] mapping		= mobjAnalysis.FactorTree.DatasetMapping;
				for (int i = 0; i < numClusters ; i++)
				{
					for (int j = 0; j<numDatasets; j++)
					{						
						tempData[i, mapping[j]] = (float)m_data[i,j];
					}
				}
				m_data = null;
				m_data = tempData;				
				mctlScatterPlot.InitScatterPlot(m_data, labelRoot);					
				*/
			}
		}
