...
using PNNLProteomics.Data.Cluster;
...
namespace MultiAlignWin
{
	...
	public class frmDataFactorSelection : System.Windows.Forms.Form
	{
		...
		private void EditSelectedFactor()
		{
			...
			classFactorTree tree = mobj_analysis.FactorTree;
			...
			foreach (clsFactor factor in tree.Factors)
			{
				if (factor.Name == factorName)
				{
					...
					foreach(MultiAlignEngine.clsDatasetInfo data in this.mobj_analysis.Files)
					{
						bool found = false;
						int i = 0;
						foreach(MultiAlignEngine.clsFactorInfo fact in data.Factors)
						{
			...
			if (result == DialogResult.OK)
			{
				...
				int indexOf = 0;
				MultiAlignEngine.clsDatasetInfo datasetInfo = mobj_analysis.Files[0] as MultiAlignEngine.clsDatasetInfo;
				for(int i = 0; i < datasetInfo.Factors.Count; i++)
				{
					MultiAlignEngine.clsFactorInfo fact = datasetInfo.Factors[i] as MultiAlignEngine.clsFactorInfo;
					if (fact.mstrFactor == factorName)
					{
						indexOf = i;
						break;
				...
				///  
				foreach(MultiAlignEngine.clsDatasetInfo data in mobj_analysis.Files)
				{
