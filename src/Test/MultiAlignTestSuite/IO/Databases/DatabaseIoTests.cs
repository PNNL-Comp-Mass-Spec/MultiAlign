        ...
        public void CreateIndex(string path)
        {
            using (var connection = new SQLiteConnection(string.Format("Data Source = {0};", path)))
            {
                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    // MSMS Mapping Table
                    command.CommandText =
                        "CREATE INDEX idx_msFeature_id on T_MSnFeature_To_MSFeature_Map(MS_Feature_ID ASC)";
                    command.ExecuteNonQuery();

                    command.CommandText =
                        "CREATE INDEX idx_msmsFeature_id on T_MSnFeature_To_MSFeature_Map(MSn_Feature_ID ASC)";
                    command.ExecuteNonQuery();
                }
