...
namespace PNNLProteomics.Data.Analysis
{
	...
	public class clsEMNormalizer
	{
		...
		double mdouble_percent_normal ; 
		double mdouble_mean_normal ; 
		...
		public double [] Normalize(double []X, double []Y, int [] common_indices)
		{
			...
			Cephes.clsMathUtilities.EMUniformRandom(logY_by_X, ref mdouble_percent_normal, ref mdouble_mean_normal) ; 
			return GetNormalizedValues(X) ; 
		}

		public double [] GetNormalizedValues(double []X)
		{
			double factor = Math.Exp(mdouble_mean_normal) ; 
