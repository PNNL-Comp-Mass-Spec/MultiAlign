...
namespace PNNLProteomics.IO.MTDB
{
    ...
    public abstract class MTSPeakMatchResultsWriter
    {
        ...
        {          
            ...
            int    redundantCount   = analysis.PeakMatchingResults.marrPeakMatchingTriplet.Length;
            int    matchMakingID    = -1;
            ...
            using (IDbConnection connection = CreateConnection(connectionString))
            {
                ...
                using (IDbCommand command = connection.CreateCommand())
                {
	                ...
	                command.Parameters.Add(CreateParameter("ComparisonMassTagCount",       analysis.MassTagDatabase.GetMassTagCount())); 
	                command.Parameters.Add(CreateParameter("UMCTolerancePPM",              0));	//TODO: do not necessarily have	if using LCMS Feature Finder Data
	                command.Parameters.Add(CreateParameter("UMCCount",                     clusters.Count));			
	                command.Parameters.Add(CreateParameter("NetAdjTolerancePPM",           analysis.DefaultAlignmentOptions.MassTolerance));		
	                command.Parameters.Add(CreateParameter("NetAdjUMCsHitCount",           clusterMatchDict.Keys.Count));		
	                ...
	                command.Parameters.Add(CreateParameter("MMATolerancePPM",              analysis.STACOptions.MassTolerancePPM));		
	                command.Parameters.Add(CreateParameter("NETTolerance",                 analysis.STACOptions.NETTolerance));			
