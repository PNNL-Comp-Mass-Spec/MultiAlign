    ...
    public abstract class clsCacheArray<T>: IDisposable where T:  new()
    {
        ...
        protected virtual void Page(long index, bool retrieve, bool cache)
        {
           // System.Diagnostics.Debug.WriteLine(String.Format("Paging cache {0},{1},{2}", index, retrieve, cache));
            ...
            /// 
            if (index >= mlng_highIndex || index < mlng_lowIndex)
            {
                ...
                ///                 
                long computedIndex  = Math.Max(index - (mlng_cacheLength / 2), 0);
                mlng_lowIndex       = Math.Min(mlng_arrayLength - mlng_cacheLength + 1, computedIndex);
		...
		public void Flush()
		{
			Cache(marr_dataCache, mlng_lowIndex, mlng_cacheLength);
		}
